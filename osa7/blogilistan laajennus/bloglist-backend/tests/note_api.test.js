...
describe('sending a new blog', () => {
    test('lengthens the list of blogs by one', async () => {
        await api.post('/api/blogs').send(testHelper.singleBlog).set({ Authorization: token }).expect(201)
    ...
    test('without the field likes adds a new blog with 0 likes', async () => {
        const response = await api.post('/api/blogs').send(testHelper.blogWithoutLikes).set({ Authorization: token }).expect(201)
    ...
    test('without the field title or url returns 400', async () => {
        await api.post('/api/blogs').send(testHelper.blogWithoutTitleOrUrl).set({ Authorization: token }).expect(400)
    })
})
...
describe('replacing a blog', () => {
    test('by id replaces the blog', async () => {
        ...
        await api.put(`/api/blogs/${blogToReplace.id}`).send(testHelper.singleBlog).expect(200)
        const blogsAtEnd = await api.get('/api/blogs')
...
describe('creating a new user', () => {
    test('with too short password or username doesnt work', async () => {
        ...
        await api.post('/api/users').send(testHelper.sampleUser).expect(400)
    ...
    test('without password or username doesnt work', async () => {
        ...
        console.log(testHelper.sampleUser)
        await api.post('/api/users').send(testHelper.sampleUser).expect(400)
        testHelper.sampleUser.username = 'asd'
