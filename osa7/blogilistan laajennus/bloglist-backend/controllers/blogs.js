...
blogsRouter.get('/:id', async (request, response) => {
    const blog = await Blog.find({ id: request.params.id }).populate('user', { username: 1, name: 1, id: 1 })
    response.json(blog)
})
...
blogsRouter.post('/:id/comments', async (request, response) => {
    ...
    blog.comments = blog.comments ? blog.comments.concat(request.body.comment) : [request.body.comment]
    const savedBlog = await blog.save()
